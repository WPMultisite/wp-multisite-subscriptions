...
class Sunrise {
	...
	public static function load() {
		...
		if ($should_startup) {
			...
			/**
			 * Define the WP Multisite Subscriptions main debug constant.
			 */
			...
			if ($security_mode) {
				...
				if (wu_get_isset($_GET, 'wu_secure') === wu_get_security_mode_key()) {
					...
					/**
					 *  Disable all plugins except WP Multisite Subscriptions
					 */
	...
	public static function try_upgrade() {
		...
		$possible_sunrises = array(
			WP_PLUGIN_DIR . '/wp-multisite-subscriptions/sunrise.php',
			WPMU_PLUGIN_DIR . '/wp-multisite-subscriptions/sunrise.php',
		);
	 ...
	 *
	 * @since 2.0.11
	 * @param array $sys_info Array containing WP Multisite Subscriptions installation info.
