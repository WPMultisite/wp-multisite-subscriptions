...
class Webhook_Manager extends Base_Manager {
	...
	protected function create_event($event_name, $id, $url, $data, $response, $is_error = false) {
		...
		if (!$is_error) {
			...
			$message .= empty($response) ? sprintf('Got response: %s', $response) : 'To debug the remote server response, turn the "Wait for Response" option on the WP Multisite Subscriptions Settings > API & Webhooks Tab';
