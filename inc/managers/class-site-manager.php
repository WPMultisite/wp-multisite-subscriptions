...
class Site_Manager extends Base_Manager {
	...
	/**
	 * Add the additional sizes required by WP Multisite Subscriptions.
	 *
	...
	public function add_no_index_warning() {
		...
		if (wu_get_setting('stop_template_indexing', false)) {
			...
			add_meta_box('wu-warnings', __('WP Multisite Subscriptions - Search Engines', 'wp-ultimo'), array($this, 'render_no_index_warning'), 'dashboard-network', 'normal', 'high');
	...
	public function render_no_index_warning() { // phpcs:disable ?>
		...
		<div class="wu-styling">
			...
			<div class="wu-border-l-4 wu-border-yellow-500 wu-border-solid wu-border-0 wu-px-4 wu-py-2 wu--m-3">
				...
				<p><?php _e('Your WP Multisite Subscriptions settings are configured to <strong>prevent search engines such as Google from indexing your template sites</strong>.', 'wp-ultimo'); ?></p>
	...
	/**
	 * Add notices to default site page, recommending the WP Multisite Subscriptions option.
	 *
	 * @since 2.0.0
	 * @return void
	 */
	public function add_notices_to_default_site_page() {
		...
		$notice = __('Hey there! We highly recommend managing your network sites using the WP Multisite Subscriptions &rarr; Sites page. <br>If you want to avoid confusion, you can also hide this page from the admin panel completely on the WP Multisite Subscriptions &rarr; Settings &rarr; Whitelabel options.', 'wp-ultimo');

		WP_Ultimo()->notices->add($notice, 'info', 'network-admin', 'wu-sites-use-wp-ultimo', array(
			array(
				'title' => __('Go to the WP Multisite Subscriptions Sites page &rarr;', 'wp-ultimo'),
