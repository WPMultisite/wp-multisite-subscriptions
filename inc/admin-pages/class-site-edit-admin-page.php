<?php
/**
 * WP Multisite Subscriptions Site Edit New Admin Page.
 *
 * @package WP_Ultimo
		...
		if (wu_request('wu-new-model')) {
			...
			\WP_Ultimo\UI\Tours::get_instance()->create_tour('new_site_template_warning', array(
				array(
					...
					'text'     => array(
						__("You just successfully added a new site template to your WP Multisite Subscriptions network and that's awesome!", 'wp-ultimo'),
		...
		$this->add_fields_widget('image', array(
			...
			'fields' => array(
				...
				'scraper_message'   => array(
					'type'            => 'note',
					'desc'            => sprintf('<span class="wu-p-2 wu-bg-red-100 wu-text-red-600 wu-rounded wu-block">%s</span>', __('We detected that this network might be running locally. If that\'s the case, WP Multisite Subscriptions will not be able to take a screenshot of the site. A site needs to be publicly available to the outside world in order for this feature to work.', 'wp-ultimo')),
