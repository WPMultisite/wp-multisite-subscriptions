<?php
/**
 * WP Multisite Subscriptions Dashboard Admin Page.
 *
 * @package WP_Ultimo
...
class Setup_Wizard_Admin_Page extends Wizard_Admin_Page {
	...
	/**
	 * Adds missing setup from settings when WP Multisite Subscriptions is not fully loaded.
	 *
	 * @since 2.0.0
	 * @return void
	 */
	...
	public function get_menu_title() {
		...
		return WP_Ultimo()->is_loaded() ? __('WP Multisite Subscriptions Install', 'wp-ultimo') : __('WP Multisite Subscriptions', 'wp-ultimo');
	...
	public function get_sections() {
		...
		$sections = array(
			'welcome'      => array(
				...
				'description' => implode('<br><br>', array(
					__('...and thanks for choosing WP Multisite Subscriptions!', 'wp-ultimo'),
					__('This quick setup wizard will make sure your server is correctly setup, help you configure your new network, and migrate data from previous WP Multisite Subscriptions versions if necessary.', 'wp-ultimo'),
			...
			'checks'       => array(
				...
				'description' => __('Now it is time to see if this machine has what it takes to run WP Multisite Subscriptions well!', 'wp-ultimo'),
			...
			'installation' => array(
				...
				'description' => __('Now, let\'s update your database and install the Sunrise.php file, which are necessary for the correct functioning of WP Multisite Subscriptions.', 'wp-ultimo'),
		...
		if ($this->is_migration()) {
			...
			if ($dry_run) {
				...
				$description = __('It seems that you were running WP Multisite Subscriptions 1.X on this network. This migrator will convert the data from the old version to the new one.', 'wp-ultimo') . '<br><br>' . __('First, let\'s run a test migration to see if we can spot any potential errors.', 'wp-ultimo');
	...
	public function renders_requirements_table() {
		...
		$plugin_requirements = array(
			...
			'wp-ultimo' => array(
				'name'              => __('WP Multisite Subscriptions', 'wp-ultimo'),
				'help'              => wu_get_documentation_url('wp-ultimo-requirements'),
	...
	/**
	 * Adds the necessary missing scripts if WP Multisite Subscriptions was not loaded.
	 *
	 * @since 2.0.0
	 * @return void
	 */
