<?php
/**
 * WP Multisite Subscriptions Customer Edit/Add New Admin Page.
 *
 * @package WP_Ultimo
...
class Customer_Edit_Admin_Page extends Edit_Admin_Page {
			...
			'new_meta_fields' => array(
				...
				'fields'            => array(
					'new_meta_remove'         => array(
						'type'            => 'note',
						'desc'            => sprintf('<a title="%s" class="wu-no-underline wu-inline-block wu-text-gray-600" href="#" @click.prevent="() => new_meta_fields.splice(index, 1)"><span class="dashicons-wu-squared-cross"></span></a>', __('Remove', 'wp-ultimo')),
		...
		$this->add_tabs_widget('options', array(
			...
			'sections' => apply_filters('wu_customer_options_sections', array(
				...
				'custom_meta'  => array(
					...
					'desc'   => __('Custom data collected via WP Multisite Subscriptions forms.', 'wp-ultimo'),
					...
					'state'  => array(
						...
						'new_meta_fields'       => array(array('type' => 'text', 'slug' => '')),
					),
				),
	...
	public function handle_save() {
		...
		foreach ($custom_meta_keys as $key => $value) {
			...
			wu_update_customer_meta($object->get_id(), $key, wu_request("meta_key_$key"), $value['type'], $value['title']);
	...
	} // end restricted_customer_meta_keys;
