...
abstract class Base_Host_Provider {
	 ...
	 *
	 * This is useful when the user is not willing to let WP Multisite Subscriptions inject the code,
	...
	public function get_constants_string($constant_values) {
		 ...
		 */
		$content = array(
			sprintf('// WP Multisite Subscriptions - Domain Mapping - %s', $this->get_title()),
		);
		 ...
		 * Adds the final line.
		 */
		$content[] = sprintf('// WP Multisite Subscriptions - Domain Mapping - %s - End', $this->get_title());
