...
class WP_Config {
 ...
 public function inject_wp_config_constant($constant, $value) {
		...
		$content = str_pad(sprintf("define( '%s', '%s' );", $constant, $value), 50) . '// Automatically injected by WP Multisite Subscriptions;';
		...
		if ($line === false) {
			...
			if ($hook_line === false) {
				...
				return new \WP_Error('unknown-wpconfig', __("WP Multisite Subscriptions can't recognize your wp-config.php, please revert it to original state for further process.", 'wp-ultimo'));
