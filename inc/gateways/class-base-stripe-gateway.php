...
class Base_Stripe_Gateway extends Base_Gateway {
	...
	public function init() {
		...
		if (method_exists('Stripe', 'setAppInfo')) {
			...
			Stripe\Stripe::setAppInfo('WordPress WP Multisite Subscriptions', wu_get_version(), esc_url(site_url()));
	 ...
	 *
	 * WP Multisite Subscriptions will call this whenever settings for this api changes.
	...
	public function install_webhook($settings, $settings_to_save, $saved_settings) {
		...
		try {
			...
			Stripe\WebhookEndpoint::create(array(
				...
				'url'            => $webhook_url,
				'description'    => 'Added by WP Multisite Subscriptions. Required to correctly handle changes in subscription status.',
			));
  ...
  * @since 2.0.0
  *
  * @param integer $customer_id WP Multisite Subscriptions customer ID.
 ...
 public function get_or_create_customer($customer_id = 0, $user_id = 0, $stripe_customer_id = 0) {
		...
		/*
		 * Use the WP Multisite Subscriptions customer ID to search on the
	 ...
	 * @since 2.0.11
	 *
	 * @param \WP_Ultimo\Objects\Billing_Address $billing_address The WP Multisite Subscriptions billing address.
	 * @return array
	 */
 ...
 protected function get_credit_coupon($cart) {
	...
	/**
	 * Converts the WP Multisite Subscriptions cart into Stripe Sub arguments.
	 *
	 * @since 2.0.0
	 *
	 * @param \WP_Ultimo\Checkout\Cart $cart The cart object.
	...
	/**
	 * Converts the Stripe invoice line items into WP Multisite Subscriptions line items.
	 *
	 * @since 2.0.19
	 *
		...
		try {
			...
			foreach ( $subscriptions->data as $subscription ) {
				...
				if ( !empty( $subscription->metadata ) ) {
					...
					// Legacy WP Multisite Subscriptions uses user_id
	...
	public function process_webhooks() {
		...
		/*
		 * Set the WP Multisite Subscriptions customer.
		 */
