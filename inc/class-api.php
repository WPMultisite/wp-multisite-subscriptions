...
/**
 * Adds a lighter ajax option to WP Multisite Subscriptions.
 *
 * @since 1.9.14
 */
class API {
    ...
    /*
		 * API & Webhooks
		 * This section holds the API settings of the WP Multisite Subscriptions Plugin.
		 */
		...
		wu_register_settings_field('api', 'api_header', array(
			...
			'desc'  => __('Options related to WP Multisite Subscriptions API endpoints.', 'wp-ultimo'),
			'type'  => 'header',
		));

		wu_register_settings_field('api', 'enable_api', array(
			'title'   => __('Enable API', 'wp-ultimo'),
			'desc'    => __('Tick this box if you want WP Multisite Subscriptions to add its own endpoints to the WordPress REST API. This is required for some integrations to work, most notabily, Zapier.', 'wp-ultimo'),
			'type'    => 'toggle',
			'default' => 1,
		));
		...
		wu_register_settings_field('api', 'api_log_calls', array(
			...
			'desc'    => __('Tick this box if you want to log all calls received via WP Multisite Subscriptions API endpoints. You can access the logs on WP Multisite Subscriptions &rarr; System Info &rarr; Logs.', 'wp-ultimo'),
			'type'    => 'toggle',
			'default' => 0,
			'require' => array(
				'enable_api' => 1,
		...
		wu_register_settings_field('api', 'webhook_header', array(
			...
			'desc'  => __('Options related to WP Multisite Subscriptions API webhooks.', 'wp-ultimo'),
			'type'  => 'header',
		));

		wu_register_settings_field('api', 'webhook_calls_blocking', array(
			'title'   => __('Wait for Response (Advanced)', 'wp-ultimo'),
			'desc'    => __('Tick this box if you want the WP Multisite Subscriptions\'s webhook calls to wait for the remote server to respond. Keeping this option enabled can have huge effects on your network\'s performance, only enable it if you know what you are doing and need to debug webhook calls.', 'wp-ultimo'),
			'type'    => 'toggle',
			'default' => 0,
		));
